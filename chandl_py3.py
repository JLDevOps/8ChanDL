...
def download_image(post):
    ...
    if chan == "8chan":
        url = "https://media.8ch.net/file_store/%s" % str(
            post["tim"]) + extension
    else:
        url = "https://i.4cdn.org/%s/%s" % (board,
                                            str(post["tim"]) + extension)
...
def main():
    ...
    parser.add_argument(
        '-url', "--url", help="The URL of the thread you want to download (4chan and 8chan supported).")
    parser.add_argument('-d', "--destination",
                        help="Where the files are to be stored.")
    parser.add_argument('-ext', "--extension", default="*",
                        help="What file extensions to download, format: ext1,ext2;...")
    parser.add_argument('-t', "--threads", default=1,
                        help="How many threads to utilise.", type=int)
    parser.add_argument('-w', "--watch", action='store_true',
                        help="Continually search for new images to download.")
    parser.add_argument('-u', "--update-interval", default=60,
                        help="Interval in seconds after which to trigger a new poll update when enabled with -w.", type=int)
    parser.add_argument('-o', "--original-filenames", action='store_true',
                        help="Whether to use the original filename of the uploaded images.")
    parser.add_argument('-gh', "--gen-hashlist", default=None,
                        help="Generate hashlist from directory.", type=str)
